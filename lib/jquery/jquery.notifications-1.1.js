...
(function($){
    ...
    $.notifications = function(msg, options) {
		...
		if (!template) {
        	template = $('<div id="jquery-notifications"></div>')
        	$(settings.templateTarget).append(template);
		}

		var n = $('<div>')
              .hide()
              .attr("id",'jquery-notifications-' + counter)
              .addClass("notification " + settings.type)
              .append($("<div>").addClass("msg").text(msg))
              .appendTo("#jquery-notifications");
		...
		if (settings.stick) {
			$('<a href="javascript:void(0);">' + settings.close + '</a>').addClass("close").click(function() {
  				if (settings.effect == "fade") {
